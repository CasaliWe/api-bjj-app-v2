...
$dados = [];
$dados['id'] = isset($_POST['id']) ? (int)$_POST['id'] : null;
...
$dados['numeroLutas'] = isset($_POST['numeroLutas']) ? (int)$_POST['numeroLutas'] : null;
$dados['numeroVitorias'] = isset($_POST['numeroVitorias']) ? (int)$_POST['numeroVitorias'] : null;
$dados['numeroDerrotas'] = isset($_POST['numeroDerrotas']) ? (int)$_POST['numeroDerrotas'] : null;
$dados['numeroFinalizacoes'] = isset($_POST['numeroFinalizacoes']) ? (int)$_POST['numeroFinalizacoes'] : null;
$dados['observacoes'] = $_POST['observacoes'] ?? null;
$dados['isPublico'] = isset($_POST['isPublico']) ? filter_var($_POST['isPublico'], FILTER_VALIDATE_BOOLEAN) : null;

// validação dos campos obrigatórios
if (empty($dados['id'])) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'O ID da competição é obrigatório.', 'errorCode' => 400]);
    exit;
}
...
// processa as imagens existentes que devem ser mantidas
$imagensExistentes = [];
if (isset($_POST['imagensExistentes']) && is_array($_POST['imagensExistentes'])) {
    foreach ($_POST['imagensExistentes'] as $id) {
        if (is_numeric($id)) {
            $imagensExistentes[] = (int)$id;
        }
    }
}

// usa o repositório para atualizar a competição
use Repositories\CompeticoesRepository;
$resultado = CompeticoesRepository::atualizar($dados, $user->id, $imagens, $imagensExistentes);
