...
// verifica se o status de destaque está presente
if (!isset($data['destacado'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'O status de destaque é obrigatório.']);
    exit;
}

// usa o repositório para alterar o destaque da técnica
use Repositories\TecnicasRepository;
$resultado = TecnicasRepository::alterarDestaque(
    $data['id'], 
    $user->id, 
    (bool)$data['destacado']
);
